...
ARCHIVE_MINIMUM=$(( 10 * 1000 * 1000 * 1000 ))
NORMAL_MINIMUM=$(( 6 * 1000 * 1000 * 1000 ))
dockermem=$(docker info --format '{{.MemTotal}}')

case "$me" in
  *privacy* )
    if [ "$dockermem" -lt $ARCHIVE_MINIMUM ]; then
      echo >&2 "This script requires that docker has at least 10GB of memory available.";
      echo >&2 "Refer to documentation to fulfill requirements."
      exit 1
    fi;
    ;;
